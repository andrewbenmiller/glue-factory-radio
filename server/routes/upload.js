...
router.post('/show', upload.array('audio', 50), async (req, res) => {
  try {
    ...
    const { title, description, trackTitle, tags: tagsRaw, series_id, episode_number, hide_episode_numbers } = req.body;
    const audioFiles = req.files || [];
    ...
    const seriesIdParsed = series_id ? parseInt(series_id, 10) : null;
    const episodeNumberParsed = episode_number ? parseInt(episode_number, 10) : null;
